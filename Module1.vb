...
Module GneraFactura
    ...
    Sub Main()
        ...
        ErrorControl = New EventLog("Application", System.Net.Dns.GetHostName(), "GeneracionCFDI")
    ...
    Sub GeneraArchivos()
        ...
        Dim ta As New GeneraFactura.ProductionDataSetTableAdapters.ClientesTableAdapter
        Dim t As New GeneraFactura.ProductionDataSet.ClientesDataTable
        Dim taMail As New ProductionDataSetTableAdapters.CorreosAnexosTableAdapter
        Dim tMail As New ProductionDataSet.CorreosAnexosDataTable
        Dim Rmail As ProductionDataSet.CorreosAnexosRow
        Dim Linea As String = ""
        Dim Mail As String = ""
        ...
        Dim taTipar As New GeneraFactura.ProductionDataSetTableAdapters.LlavesTableAdapter
    ...
    Private Sub EnviaError(ByVal Para As String, ByVal Mensaje As String, ByVal Asunto As String)
        If InStr(Mensaje, "No se ha encontrado la ruta de acceso de la red") = 0 Then
            Dim Mensage As New MailMessage("GeneraFactura@cmoderna.com", Trim(Para), Trim(Asunto), Mensaje)
    ...
    Sub GeneraArchivosAvio()
        ...
        Dim Fega As Double
        Dim Razon As String = ""
        Dim FegaAux As Double
        Dim f1 As System.IO.StreamWriter
        Dim fecha As New DateTime
        Dim Facturas As New GeneraFactura.ProductionDataSetTableAdapters.FacturasAvioTableAdapter
        Dim FAC As New GeneraFactura.ProductionDataSet.FacturasAvioDataTable
        Dim Detalles As New GeneraFactura.ProductionDataSetTableAdapters.FacturasAvioDetalleTableAdapter
        Dim DET As New GeneraFactura.ProductionDataSet.FacturasAvioDetalleDataTable
        Dim Folios As New GeneraFactura.ProductionDataSetTableAdapters.LlavesTableAdapter
        Dim tasa As Double
        Dim Consecutivo As Integer
        Dim RegAfec As Integer
        Dim SubTTAux As Double
        Dim IVAAux As Double
        Dim IVAAuxFega As Double
        ...
        For Each rr As GeneraFactura.ProductionDataSet.FacturasAvioDetalleRow In DET.Rows
        ...
        ''For Each r As GeneraFactura.ProductionDataSet.FacturasAvioRow In FAC.Rows
        ''    Estado_de_Cuenta_Avio(r.Anexo, r.Ciclo, 0, "Facturas", r.InteresMensual) 'CALCULA EL ESTADIO DE CEUNTA ANTES DE FACTURAR
        ''Next
        For Each r As GeneraFactura.ProductionDataSet.FacturasAvioRow In FAC.Rows
            Consecutivo = Folios.SerieGVA
            'If r.Anexo = "085870005" And r.FechaInicial = "20140703" Then
            ''Try
            Console.WriteLine("Generando CFDI AVIO..." & r.Anexo & " " & Consecutivo)
            For x = 0 To 20
                Arre(1, x) = ""
                Arre(2, x) = ""
            Next
            ...
            IVAAux = 0
            IVAAuxFega = 0
            f1 = New System.IO.StreamWriter(GeneraFactura.My.Settings.RutaCFDI & "CFDI-AV-" & Consecutivo & "-" & r.Anexo & ".txt", False, Text.Encoding.GetEncoding(1252))
            'f1 = New System.IO.StreamWriter(My.Computer.FileSystem.CurrentDirectory & "\CFDI-AV-" & Consecutivo & ".txt", False, Text.Encoding.GetEncoding(1252))
            EsNotaCredito = False
            f1.WriteLine("#InicioComprobante")
            f1.WriteLine("idn:documento                   =        FACTURA")
            f1.WriteLine("idn:tipofactura                 =        FINANZAS")
            ...
            f1.WriteLine("idn:documanager                 =        FINAGIL")
            f1.WriteLine()
            f1.WriteLine("fsc:serie                       =        AV")
            f1.WriteLine("fsc:folio                       =        " & Consecutivo)
            f1.WriteLine("fsc:fecha                       =        " & fecha.ToString("yyyy-MM-dd") & "T" & fecha.ToString("HH:mm:ss"))
            f1.WriteLine("fsc:formaDePago                 =        PAGO EN UNA SOLA EXHIBICION")
            f1.WriteLine("fsc:noCertificado               =        00001000000202240016")
            f1.WriteLine("fsc:condicionesDePago           =        ")
            ...
            End If
            If r.Tipar = "C" Then
                'f1.WriteLine("fsc:metodoDePago                =        Mediante crédito en Cuenta Corriente")
            Else
                'f1.WriteLine("fsc:metodoDePago                =        Mediante crédito de Avío")
            End If
            f1.WriteLine("fsc:metodoDePago                =        NA")

            f1.WriteLine("fsc:LugarExpedicion             =        Toluca, México")
            f1.WriteLine("fsc:NumCtaPago                  =        ")
            f1.WriteLine()
            f1.WriteLine("#Emisor")
            f1.WriteLine("fem:rfc                         =        FIN940905AX7")
            f1.WriteLine("fem:nombre                      =        FINAGIL S.A. DE C.V, SOFOM E.N.R")
            ...
            f1.WriteLine("frd:localidad                   =        ")
            f1.WriteLine("frd:municipio                   =        " & r.Delegacion)
            f1.WriteLine("frd:estado                      =        " & r.Estado)
            f1.WriteLine("frd:pais                        =        México")
            f1.WriteLine("frd:codigopostal                =        " & r.Copos)
            f1.WriteLine()
            f1.WriteLine("#Detalle")
            'f1.WriteLine(" dco:cant  dco:unit   dco:noId         dco:desc                       dco:vUni      dco:impo       dcd:line dcd:unid dcd:kg     dcd:prom  dcd:coAl")
            f1.WriteLine(" dco:cant  dco:unit   dco:noId         dco:desc                                                              dco:vUni      dco:impo       dcd:line dcd:unid dcd:kg     dcd:prom  dcd:coAl")
            ...
            For Each rr As GeneraFactura.ProductionDataSet.FacturasAvioDetalleRow In DET.Rows
                ...
                Or Trim(rr.Concepto) = "ANALISIS DE SUELOS" Then
                    SubTTAux = 0
                    IVAAux = 0
                    IVAAuxFega = Math.Round(rr.IvaFega, 4)
                    FegaAux = Math.Round(rr.FEGA, 4)

                    IVA += IVAAux + IVAAuxFega
                    TOt += SubTTAux + IVAAux + IVAAuxFega + FegaAux
                    Fega += FegaAux
                    SubTT += SubTTAux
                Else
                    x += 1
                    SubTTAux = Math.Round(rr.Importe, 4)
                    IVAAux = Math.Round(rr.IvaImporte, 4)
                    IVAAuxFega = Math.Round(rr.IvaFega, 4)
                    FegaAux = Math.Round(rr.FEGA, 4)
                    ...
                    Impo = SubTTAux
                    'f1.WriteLine(Space(8) & "1   SER" & Space(24) & Mid(Trim(Concep), 1, 30) & Space(30 - Trim(Mid(Concep, 1, 30)).Length) & Space(13 - Impo.Length) & Impo & Space(15 - Impo.Length) & Impo & Space(20 - Impo.Length) & "  ")
                    f1.WriteLine(Space(8) & "1   SER" & Space(24) & Mid(Trim(Concep), 1, 70) & Space(70 - Trim(Mid(Concep, 1, 70)).Length) & Space(13 - Impo.Length) & Impo & Space(15 - Impo.Length) & Impo & Space(20 - Impo.Length) & "  ")
                    IVA += IVAAux + IVAAuxFega
                    TOt += SubTTAux + IVAAux + IVAAuxFega + FegaAux
                    Fega += FegaAux
                    SubTT += SubTTAux
                ...
                RegAfec = Facturas.Facturar("AV" & Consecutivo, r.Anexo, r.Ciclo, r.FechaFinal, Trim(rr.Concepto))
                If RegAfec = 0 Then
            ...
            If Fega > 0 Then
                ...
                Arre(1, x) = Mid(Trim(Concep), 1, 70)
                Impo = Math.Round(Fega, 4)
                'f1.WriteLine(Space(8) & "1   SER" & Space(24) & Mid(Trim(Concep), 1, 30) & Space(30 - Trim(Mid(Concep, 1, 30)).Length) & Space(15 - Impo.Length) & Impo & Space(15 - Impo.Length) & Impo & Space(20 - Impo.Length) & "  ")
                f1.WriteLine(Space(8) & "1   SER" & Space(24) & Mid(Trim(Concep), 1, 70) & Space(70 - Trim(Mid(Concep, 1, 70)).Length) & Space(13 - Impo.Length) & Impo & Space(15 - Impo.Length) & Impo & Space(20 - Impo.Length) & "  ")
                Arre(2, x) = "*" & Format(Fega, "#,##0.00")
                SubTT += Fega
            End If
            If IVA > 0 Then tasa = 16 Else tasa = 0
            ...
            f1.WriteLine()
            f1.WriteLine("agr:noCliente                   =       " & r.Cliente)
            f1.WriteLine("agr:fechaOrdenCompra            =")
            f1.WriteLine("agr:fechaDeContraReciboMercancia=       " & Mid(r.FechaFinal, 1, 4) & "-" & Mid(r.FechaFinal, 5, 2) & "-" & Mid(r.FechaFinal, 7, 2))
            f1.WriteLine("agr:tipoMoneda                  =")
            f1.WriteLine("agr:totalKilos                  =")
            f1.WriteLine("agr:telefonoCliente             =")
            f1.WriteLine("agr:comentariosLeyenda          =        " & r.AnexoCon & " " & r.CicloPagare)
            f1.WriteLine("agr:LeyendaP                    =        EL PAGO DE ESTE DOCUMENTO SE HACE EN UNA SOLA EXHIBICION")
            f1.WriteLine("agr:nopolizaSeg                 =        " & r.Anexo & "-" & r.Ciclo & "-" & r.FechaFinal)
            ...
            f1.WriteLine("adi:mailagente                  =")
            f1.WriteLine("adi:ImpresoraLocal              =")
            f1.WriteLine("adi:Condicion                   =         NA")

            If r.Cliente = "05978 " Then
                f1.WriteLine("adi:Mail1                       =         " & Trim(r.EMail1))
                f1.WriteLine("adi:Mail2                       =         " & Trim(r.EMail2))
                f1.WriteLine("adi:Mail3                       =         flen.estrada@ciasaconstruccion.com.mx")
                f1.WriteLine("adi:Mail4                       =         administacion@ciasaconstruccion.com.mx")
            Else
                f1.WriteLine("adi:Mail1                       =")
                f1.WriteLine("adi:Mail2                       =")
                f1.WriteLine("adi:Mail3                       =")
            ...
            If r.Tipar <> "C" Then
                Select Case Trim(r.Sucursal)
                    Case "MEXICALI"
                        f1.WriteLine("adi:Mail5                       =         sduarte@finagil.com.mx")
                    Case "NAVOJOA"
                        f1.WriteLine("adi:Mail5                       =         mlopezb@finagil.com.mx")
                    Case "IRAPUATO"
                        f1.WriteLine("adi:Mail5                       =         vtezcuc@finagil.com.mx")
                    Case Else
                        f1.WriteLine("adi:Mail5                       =         vcruz@finagil.com.mx")
                End Select
            Else
                f1.WriteLine("adi:Mail5                       =         vcruz@finagil.com.mx")
            End If
            ...
            If EsNotaCredito = True Then
                ...
                f1.WriteLine("iit:impuesto                    =        IVA")
                f1.WriteLine("iit:tasa                        =        ")
                f1.WriteLine("iit:importe                     =" & Space(26 - IVA.ToString.Length) & TOt.ToString)
                f1.WriteLine()
                f1.WriteLine()
                f1.WriteLine("#EntregaEn")
                f1.WriteLine("aen:nombre                      =")
                ...
                f1.WriteLine()
                f1.WriteLine("#Totales")
                f1.WriteLine("ato:subtotalSinDescuentoSinIva  =" & Space(26 - TOt.ToString.Length) & TOt.ToString)
                f1.WriteLine("ato:cantidadConLetra            =       " & Letras(TOt.ToString, "MXN"))
                f1.WriteLine()
                f1.WriteLine("agr:noCliente                   =       " & r.Cliente)
                f1.WriteLine("agr:fechaOrdenCompra            =")
                ...
                f1.WriteLine("agr:totalKilos                  =")
                f1.WriteLine("agr:telefonoCliente             =")
                f1.WriteLine("agr:comentariosLeyenda          =" & r.AnexoCon)
                f1.WriteLine("agr:LeyendaP                    =        EL PAGO DE ESTE DOCUMENTO SE HACE EN UNA SOLA EXHIBICION")
                f1.WriteLine()
                f1.WriteLine("adi:impresora                   =")
                f1.WriteLine("adi:email                       =")
    ...
    Sub GeneraArchivosEXternas()
        ...
        Dim f1 As System.IO.StreamWriter
        Dim Facturas As New GeneraFactura.ProductionDataSetTableAdapters.FacturasExternasTableAdapter
        Dim FAC As New GeneraFactura.ProductionDataSet.FacturasExternasDataTable
        Dim Detalles As New GeneraFactura.ProductionDataSetTableAdapters.FacturasExternasDETTableAdapter
        Dim DET As New GeneraFactura.ProductionDataSet.FacturasExternasDETDataTable
        Dim tasa As Double
        Dim SubTTAux As Double
        Dim IVAAux As Double
        Dim Concep As String
        ...
        Dim MetodoPago As String = ""
        Dim taMetodo As New ProductionDataSetTableAdapters.LlavesTableAdapter


        Facturas.Fill(FAC)
        For Each r As GeneraFactura.ProductionDataSet.FacturasExternasRow In FAC.Rows
            ...
            For Each rr As GeneraFactura.ProductionDataSet.FacturasExternasDETRow In DET.Rows
                x += 1
                SubTTAux = Math.Round(rr.Importe, 4)
